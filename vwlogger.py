...
CONST_LOGGING_FMT 	: Final[str]	= '[%(levelname)s][%(asctime)s][%(module)s.%(funcName)s]-%(message)s]'
...
def trace(func):
    ...
    def trace_wrapper(*args, **kwargs):
        #logging.info(f"{func.__name__} {CONST_STR_SEP} {CONST_STR_STARTED} {CONST_STR_SEP} {args.__str__():.60}..")
        logging.info(f"{CONST_STR_ARROW}[{func.__name__}]{CONST_STR_SEP}{CONST_STR_STARTED} {CONST_STR_SEP} {args.__str__():.60}..")
        ret = func(*args, **kwargs)
        #logging.info(f"{func.__name__} {CONST_STR_SEP} {CONST_STR_END}")
...
def performance(func):
	...
	def perf_wrapper(*args, **kwargs):
		s = time.perf_counter()
		ret = func(*args, **kwargs)
		#logging.info(f"{func.__name__} {CONST_STR_SEP} {CONST_STR_EXEC_TIME} {CONST_STR_SEP} {time.perf_counter()-s}")
		logging.info(f"{CONST_STR_ARROW}[{func.__name__}]{CONST_STR_SEP}{CONST_STR_EXEC_TIME} {CONST_STR_SEP} {time.perf_counter()-s}")
		return ret
	return perf_wrapper
